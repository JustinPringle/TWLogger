 ...
 ***************************************************************************/
#include <TW_LSM9DS0.h>

/***************************************************************************
 CONSTRUCTOR
 ***************************************************************************/

void TW_LSM9DS0::initI2C( TwoWire* wireBus, int32_t sensorID ) {
    _i2c = true;
    _wire = wireBus;
    ...
    _accelSensor = Sensor(this, &TW_LSM9DS0::readAccel, &TW_LSM9DS0::getAccelEvent, &TW_LSM9DS0::getAccelSensor);
    _magSensor   = Sensor(this, &TW_LSM9DS0::readMag,   &TW_LSM9DS0::getMagEvent,   &TW_LSM9DS0::getMagSensor);
    _gyroSensor  = Sensor(this, &TW_LSM9DS0::readGyro,  &TW_LSM9DS0::getGyroEvent,  &TW_LSM9DS0::getGyroSensor);
    _tempSensor  = Sensor(this, &TW_LSM9DS0::readTemp,  &TW_LSM9DS0::getTempEvent,  &TW_LSM9DS0::getTempSensor);
}


// default
TW_LSM9DS0::TW_LSM9DS0( int32_t sensorID ) {
    initI2C(&Wire, sensorID);
}

TW_LSM9DS0::TW_LSM9DS0( TwoWire* wireBus, int32_t sensorID ) {
    initI2C(wireBus, sensorID);
}

TW_LSM9DS0::TW_LSM9DS0(int8_t xmcs, int8_t gcs, int32_t sensorID ) {
  _i2c = false;
  // hardware SPI!
  _csg = gcs;
  _csxm = xmcs;
...
TW_LSM9DS0::TW_LSM9DS0(int8_t clk, int8_t miso, int8_t mosi, int8_t xmcs, int8_t gcs, int32_t sensorID ) {
  _i2c = false;
  // software SPI!
  _csg = gcs;
  _csxm = xmcs;
...
//   Serial.println(id, HEX);
  ...
  */
  ...
  //write8(XMTYPE, LSM9DS0_REGISTER_CTRL_REG3_XM, 0x04);
  write8(XMTYPE, LSM9DS0_REGISTER_CTRL_REG5_XM, 0b11110000); //MAG 50HZ, temp sensor disabled
  ...
  #else
  write8(GYROTYPE, LSM9DS0_REGISTER_CTRL_REG1_G,0b00000000);
...
void TW_LSM9DS0::setupAccel ( lsm9ds0AccelRange_t range )
{
...
void TW_LSM9DS0::setupMag ( lsm9ds0MagGain_t gain )
{
...
void TW_LSM9DS0::setupGyro ( lsm9ds0GyroScale_t scale )
{
...
*/
/**************************************************************************/
bool TW_LSM9DS0::getEvent(sensors_event_t *accelEvent,
...
*/
/**************************************************************************/
void TW_LSM9DS0::getSensor(sensor_t *accel, sensor_t *mag,
...
/***************************************************************************
 PRIVATE FUNCTIONS
 ***************************************************************************/
void TW_LSM9DS0::write8(boolean type, byte reg, byte value)
{
  byte address, _cs;
...
byte TW_LSM9DS0::read8(boolean type, byte reg)
{
  uint8_t value;
...
byte TW_LSM9DS0::readBuffer(boolean type, byte reg, byte len, uint8_t *buffer)
{
  byte address, _cs;
...
uint8_t TW_LSM9DS0::spixfer(uint8_t data) {
...
void TW_LSM9DS0::getAccelEvent(sensors_event_t* event, uint32_t timestamp) {
...
void TW_LSM9DS0::getMagEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
...
void TW_LSM9DS0::getGyroEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
  event->sensor_id = _lsm9dso_sensorid_accel;
...
void TW_LSM9DS0::getTempEvent(sensors_event_t* event, uint32_t timestamp) {
  memset(event, 0, sizeof(sensors_event_t));
  event->version   = sizeof(sensors_event_t);
...
void TW_LSM9DS0::getAccelSensor(sensor_t* sensor) {
...
void TW_LSM9DS0::getMagSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
...
void TW_LSM9DS0::getGyroSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
...
void TW_LSM9DS0::getTempSensor(sensor_t* sensor) {
  memset(sensor, 0, sizeof(sensor_t));
